...
SOURCES =  param.mli param.ml data.ml md.ml formula.ml generate.ml main.ml
# SOURCES = lexer.mll parser.mly types.mli types.ml main.ml
...
tarball: clean
	rm -rf bootstrap
	mkdir bootstrap
	mkdir bootstrap/exemples/
	touch bootstrap/.depend
	cp *.ml *.mli test.sh Makefile README.md LICENSE md5 minisat bootstrap/
	cp exemples/*.hex exemples/*.txt bootstrap/exemples/
